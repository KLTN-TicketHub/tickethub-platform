...
namespace Catalog.Application.Features.Events.Queries.GetEventById
{
    public class GetEventByIdQueryHandler : IRequestHandler<GetEventByIdQuery, EventDto>
    {
        ...
        public GetEventByIdQueryHandler(IUnitOfWork unitOfWork, IFileService fileService)
        {
            _unitOfWork = unitOfWork;
            _fileService = fileService;
        }

        public async Task<EventDto> Handle(GetEventByIdQuery query, CancellationToken cancellationToken = default)
        {
            return await GetEventByIdAsync(query.Id, cancellationToken);
        }

        private async Task<EventDto> GetEventByIdAsync(Guid id, CancellationToken cancellation = default)
        {
            return await _unitOfWork.EventRepository.GetOneUntrackedAsync(
                filter: e => e.Id == id && !e.IsDeleted && e.Status == EventStatus.Published && e.EndAt >= DateTime.UtcNow,
                selector: e => new EventDto
                {
                    ...
                    ReasonForRejection = e.Status == EventStatus.Rejected && e.Approval != null ? e.Approval.Reason : null
