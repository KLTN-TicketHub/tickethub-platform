...
namespace Identity.Application.Features.Admin.Users.Commands.UpdateUser
{
    public class UpdateUserCommandHandler : IRequestHandler<UpdateUserCommand, UserDetailDto>
    {
        ...
        {
            return await UpdateUserAsync(command.Id, command.Request, cancellationToken);
        }

        private async Task<UserDetailDto> UpdateUserAsync(Guid id, UpdateUserRequest request, CancellationToken cancellationToken)
        {
            ...
            if (!updateResult.Succeeded)
                throw new BusinessRuleException(string.Join(", ", updateResult.Errors.Select(e => e.Description)));

            await CreateAuditLogAsync("Updated", user.Id, cancellationToken);
