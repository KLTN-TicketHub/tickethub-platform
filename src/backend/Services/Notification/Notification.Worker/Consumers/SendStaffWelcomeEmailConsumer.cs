...
{
    public class SendStaffWelcomeEmailConsumer : IConsumer<StaffRegisteredEvent>
    {
        private readonly ILogger<SendStaffWelcomeEmailConsumer> _logger;
        private readonly MailServiceFactory _mailServiceFactory;
        private readonly AppUrls _appUrls;

        public SendStaffWelcomeEmailConsumer(
            ILogger<SendStaffWelcomeEmailConsumer> logger,
        ...
        public async Task Consume(ConsumeContext<StaffRegisteredEvent> context)
        {
            var message = context.Message;

            _logger.LogInformation(
                "Received StaffRegisteredEvent: UserId={UserId}, Email={Email}, CorrelationId={CorrelationId}",
                message.UserId,
                message.Email,
            ...
            {
                To = message.Email,
                Subject = "Chào mừng bạn đến với TicketHub — Thông tin tài khoản nhân viên soát vé",
            ...
            try
            {
                ...
                _logger.LogInformation(
                    "Staff welcome email sent for UserId={UserId}, CorrelationId={CorrelationId}",
                    message.UserId,
                    message.CorrelationId);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to send staff welcome email for UserId={UserId}", message.UserId);
                throw;
            }
        }

        private static string BuildWelcomeEmailBody(StaffRegisteredEvent message, string feBaseUrl)
        {
...
<div style="font-family: Arial, sans-serif; line-height: 1.6; color: #111827; max-width: 600px; margin: 0 auto;">
    ...
    <p>Tài khoản <strong>nhân viên soát vé</strong> của bạn đã được nhà tổ chức tạo trên hệ thống TicketHub.</p>
