  ...
  >
    ...
    <div class="p-6 px-8 border-b border-border-main flex items-center justify-between">
      <router-link to="/" class="font-heading text-xl font-bold text-main tracking-tight">
        Event<span class="text-primary">Sphere</span> <span class="text-[10px] uppercase tracking-widest text-muted ml-1 opacity-50">Moderator</span>
      </router-link>
    ...
    <div class="p-4 border-t border-border-main">
      ...
      >
        ...
        <span class="text-[14px]">Thoát Moderator</span>
      </button>
    </div>
  </aside>
</template>
...
const menuItems = [
  ...
  { 
    ...
    icon: () => h('svg', { fill: 'none', stroke: 'currentColor', 'stroke-width': '2', viewBox: '0 0 24 24' }, [
      h('path', { d: 'M4 6h16M4 12h16M4 18h7' }),
    ])
  },
]
...
const handleLogout = async () => {
  await authLogout()
  if (router.currentRoute.value.path.startsWith('/moderator')) {
